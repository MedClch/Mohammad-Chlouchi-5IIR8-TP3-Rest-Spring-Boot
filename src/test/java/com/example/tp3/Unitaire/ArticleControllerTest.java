package com.example.tp3.Unitaire;
...
public class ArticleControllerTest {
    ...
    void testGetAll() throws Exception{
        ...
        mvc.perform(get("/api/articles/all").contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(1L))
                .andExpect(jsonPath("$[1].desc").value("ECRAN"))
                .andExpect(jsonPath("$[2].price").value(3000d))
                .andExpect(jsonPath("$[3].qte").value(10d));
    }
}
